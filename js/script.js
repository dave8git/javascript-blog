...
function generateTags() {
  ...
  for (let article of articles) {// START LOOP: for every article: 
    ...
    for (let splitTag of splitTags) { // START LOOP: for each tag 
      let htmlLink = '<li><a class="' + optCloudClassPrefix + '5' + '"' + ' ' + 'href="#tag-' + splitTag + '">' + splitTag + '</a></li>' + ' '; // generate HTML of the link 
  ...
  for(let tag in allTags) { // START LOOP: for each tag in allTags
    allTagsHTML += tag + ' (' + allTags[tag] + ') '; // generate code of a link and add it to allTagsHTML
